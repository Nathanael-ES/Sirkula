...
<a href="{{ route('distributions.create') }}" class="btn btn-success mb-3 rounded-pill px-3">{{ __('messages.add_distribution') }}</a>
    ...
    <tbody>
        @forelse($distributions as $dist)
            <tr>
                ...
                <td>
                    <form action="{{ route('distributions.destroy', $dist->id) }}" method="POST" class="d-inline">@csrf @method('DELETE')<button class="btn btn-sm btn-danger bi bi-trash">{{ __('messages.delete') }}</button></form>
                </td>
            </tr>
        @empty
