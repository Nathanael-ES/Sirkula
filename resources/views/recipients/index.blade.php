...
<a href="{{ route('recipients.create') }}" class="btn btn-success mb-3">{{ __('messages.add_recipient') }}</a>
    ...
    <tbody>
        @forelse($recipients as $r)
            <tr>
                <td>{{ $r->name }}</td>
                <td>
                    <a href="{{ route('recipients.edit', $r->id) }}" class="btn btn-sm btn-warning">{{ __('messages.edit') }}</a>
                    <form action="{{ route('recipients.destroy', $r->id) }}" method="POST" class="d-inline">@csrf @method('DELETE')<button class="btn btn-sm btn-danger">{{ __('messages.delete') }}</button></form>
                </td>
            </tr>
        @empty
